...
ui <- dashboardPage(
	...
	## SIDEBAR MENU
	dashboardSidebar(
		width = 220, sidebarMenu(br(), menuItem("Collecting Data", tabName = "collecting", icon = icon("file")),
				menuItem("Quality Control", tabName = "quality", icon = icon("thumbs-up")),	
				menuItem("Selection Settings", tabName = "adj", icon = icon("gears"),
					selectInput("annotation", "Default annotation file", choices = list("I want insert my .annot file"="other", 
						"mogene10sttranscriptcluster"="mogene10sttranscriptcluster.db", "hgu133a"="hgu133a.db",
					...
					radioButtons("groups", label="Groups to compare", inline = TRUE, choices = list("Group 1 vs Group 2" = "1", 
						"Group 1 vs Group 3" = "2", "Group 1 vs Group 4" = "4", "Group 2 vs Group 3" = "3", "Group 2 vs Group 4" = "5", 
				...
				menuItem("Selection Settings GO", tabName = "adjGO", icon = icon("gears"),
					selectInput("pvalGO", "P. value", choices = list(0.001, 0.005, 0.01,0.05,0.1,0.5,1), selected = 0.01, multiple = FALSE,
						selectize = TRUE, width = NULL, size = NULL),
					radioButtons("ontol", label="Ontology", inline = TRUE, choices = list("CC (Cellular component)" = "CC", 
						"MF (Molecular function)" = "MF", "BP (Biological process)" = "BP"), selected = "BP")),
	...
	dashboardBody(
		...
		tabItems( tabItem(tabName = "collecting", h1("Collecting Data"), h3("Please, select the path of the '.CEL' file and the target file"), 
			br(), fluidPage(splitLayout(textInput("CEL", "Path .CEL files", placeholder = "Path"),
			...
			fluidPage(splitLayout(
				box(title = "Files .CEL", status = "warning", solidHeader = TRUE, tableOutput("celfiles"), height = 450, width = 1500),
				box(title = "Target File", status = "warning", solidHeader = TRUE, tableOutput("targetfile"), height = 450, width = 1500)
				)
			),
			fluidPage(splitLayout( h4("Number of groups: "), textOutput("gro"), h4("Groups: "), textOutput("grou")))),
			tabItem(tabName = "quality", h1("Quality control"), br(), fluidPage(splitLayout(
				box(title = "Boxplot before normalization", status = "warning", solidHeader = TRUE,	plotOutput("box_bef"), height = 475, width = 600),
				box(title = "Boxplot after normalization", status = "warning", solidHeader = TRUE, plotOutput("box_aft"), height = 475, width = 600)
				)
			),
			fluidPage(splitLayout(
				box(title = "Dendogram before normalization", status = "warning", solidHeader = TRUE,plotOutput("den_bef"), height = 475, width = 600),
				box(title = "Dendogram after normalization", status = "warning", solidHeader = TRUE, plotOutput("den_aft"), height = 475, width = 600))
				)
			),
			tabItem(tabName = "selection", h1("Results"), br(),	h4("Only the first items will be displayed. You can download the complete table."),fluidPage(splitLayout(
				box(title = "Top Table",status = "warning", solidHeader = TRUE, tableOutput("selected1"), downloadButton("dwtable", "Download Genes"),  height = 450, width = 500),
				box(title = "Selected", status = "warning", solidHeader = TRUE,	tableOutput("selected2"), downloadButton("dwtable2", "Download Genes"), height = 450, width = 500) 
				)
			),
			fluidPage(splitLayout(
				box(title = "Up-regulated Genes", status = "warning", solidHeader = TRUE, tableOutput("upgenes"), downloadButton("dwup", "Download Up Genes"), height = 450, width = 500), 
				box(title = "Down-regulated Genes", status = "warning", solidHeader = TRUE, tableOutput("downgenes"), downloadButton("dwdw", "Download Down Genes"), height = 450, width = 500)
				)
			),
			fluidPage(splitLayout(
				box(title = "Volcano Plot", status = "warning", solidHeader = TRUE, plotOutput("volcan"), height = 475, width = 500), 
				box(title = "Heat Map", status = "warning", solidHeader = TRUE, plotOutput("heatm"), height = 475, width = 500)
				)
			),
			textOutput("prueba")),
			tabItem(tabName = "ontology2", h1("Gene Ontology Analysis"), br(),h4("Only the first items will be displayed. You can download the complete table (File .html)"),
				box(title = "Ontology", status = "warning", solidHeader = TRUE, tableOutput("go2") , downloadButton("dwgo2", "Download table"), height = 750, width = 1000)
								#          tableOutput("go2"), downloadButton("dwgo2", "Download table"), height = 750, width = 1000)
			),
			tabItem(tabName = "ontology",h1("Gene Annotation"),	br(),h4("Only the first items will be displayed. You can download the complete table."),
				box(title = "Annotation", status = "warning", solidHeader = TRUE, tableOutput("go"), downloadButton("dwgo", "Download table"), height = 750, width = 1000)
			),
			tabItem(tabName = "help",h2("What is this?"),p("The purpose of this application is to help people with little knowledge of bioinformatics to perform simple microarray analysis. Developed by Isaac Morales General. Microarray Analyzer v1.0."),
			h2("How can you use it?"), p("You need to perform this steps to analyse your data:"),
				tags$ol(
					tags$li(p(strong("Collecting data: ")," You need to indicate: "), 
						tags$ul(
							tags$li(p(strong("Path .cel files: "), "Indicate the path in which you have storaged your .cel files"))), 
						tags$ul(
							tags$li(p(strong("Target file: "), "The targets is a file where you defines the levels of each variable under study. 
								The application will use this file to colour the samples in the plot. Upload a .csv file  with your variables
								...
								of the sample and in the last column the colour to identify them in the plot. Every column must be separated by commas. See",
					...
					tags$li(p(strong("Selection settings: "), "We need to configure some parameters before run the application"),
						...
						tags$ul(tags$ul(tags$li(p("If you select the .annot file, you must remove the head. See examples: ",
														tags$a(href="annot11.jpg", target="_blank", "Before remove"),(" / "),
														tags$a(href="annot21.jpg", target="_blank", "After remove"))))),
						tags$ul(tags$li(p(strong("Select the groups to compare:"), "Here you can choose which groups you wish to compare"))),
						tags$ul(tags$li(p(strong("Select LFC:"), "Here you have to put the minimum absolute log2-fold-change required. (0-10)")))),
						tags$ul(tags$li(p(strong("Select Adj. p-val:"), "Select the cutoff value for adjusted p-values. Only genes with lower p-values are listed"))),
						tags$ul(tags$li(p(strong("Select the number of genes (Volcano Plot):"), "Select the number of genes showed in the Volcano Plot"))),
						tags$ul(tags$li(p(strong("Select the number of genes (Heatmap):"), "Select the number of genes showed in the Heatmap"))),
...
server <- function(input, output) {
	...
	# Read CEL files
	data <- reactive({
		validate(need(input$CEL != "", "Please select a data set"))
	...
	# Read target file
	phenodata <- reactive({
		validate(need(input$target != "", "Please select a target file"))
		file1 = input$target
	...
	output$dwtable <- downloadHandler(filename = function() {"TableGenes.csv" },
	...
	output$dwup <- downloadHandler(filename = function() { "TableGenes.csv" },
	content = function(file) {
		res <- selected()
		topups = res[res[, "logFC"] > 1, ]
	...
	output$dwdw <- downloadHandler(filename = function() { "TableGenes.csv" },
	content = function(file) {
		res <- selected()
		topdown = res[res[, "logFC"] < -1, ]
	...
	output$dwgo2 <- downloadHandler(filename = function() { "myreport.html" },
	content = function(file) {
		require(GOstats)
		annota <- input$annotation
		bd <- character()
	...
	output$dwgo <- downloadHandler(filename = function() { "TableGenes.csv" },
	content = function(file) {
		res <- selected()
		a <- as.vector(res$Gene.symbol)
		annota <- input$annotation
